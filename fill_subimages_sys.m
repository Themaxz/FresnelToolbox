function [fillStack num newpix]=fill_subimages_sys(raw,pix,gr)
...
endstack=size(STACK,3);
k=1;
 halfPix = round(pix/2);
 cropStack = STACK(end/2 -halfPix/2:end/2 +halfPix/2,end/2 -halfPix/2:end/2 +halfPix/2,:);
 for k=1:endstack
     buffer=mean(cropStack(:,:,k));
     meanvalues(k)=mean(buffer(:));
    
 end
 fillStack=zeros(pix,pix,endstack);
 for k=1:endstack  
     fillStack(:,:,k)=ones(pix,pix).*meanvalues(k);
 end
 fillStack(end/2 -halfPix/2:end/2 +halfPix/2,end/2 -halfPix/2:end/2 +halfPix/2,:)=cropStack;
 newpix = size(fillStack,1);
 num=[s1-1 s2-1];
