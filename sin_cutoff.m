...
if def==0
    [SIN1,image1]=Pleno20_SIN(P,L,M,fmin,fmin,a,b,lambda,iterations,1,'y',freq);
else
...
figure, plot(x,p2,'r');
legend('image','rendered');
saveas(figure(1),strcat(date,'image_cutoff',num2str(m),'-',num2str(freq)),format);
saveas(figure(2),strcat(date,'renderfi_cutoff',num2str(m),'-',num2str(freq)),format);

figure, imagesc(x,x,image1),map;
xlabel('position (m)');
ylabel('position (m)');
title('Main Lens Image');
saveas(figure(3),'mainimg_cutoff','fig');
saveas(figure(3),'mainimg_cutoff',format);

figure, imagesc(x,x,img),map;
xlabel('position (m)');
ylabel('direction (m)');
title('rendered Image');
saveas(figure(4),'renderimg_cutoff','fig');
saveas(figure(4),'renderimg_cutoff',format);
