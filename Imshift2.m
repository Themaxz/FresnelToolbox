function [IMG,P,K]=Imshift2(A,alpha,num_lenslet)
s=max(size(A));
a=(1-1./alpha); %shift
u_max=s./num_lenslet; %pixel direzinali sottesi da ogni microlente
v_max=u_max; %e anche pari al numero di subimmagini
pace=num_lenslet;
nu=[-100-pace./2:1:pace./2+100]; %vettore shift in pixel dimensione di ogni subimmagine
[NU, ETA]=meshgrid(nu);
U=[-u_max./2:1:u_max./2];
V=U;
U=U.*a; %vettori shift
