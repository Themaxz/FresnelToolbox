function Y = randphase_sources1(coherence_index, dimension)

%Y=randphase_sources(coherence_index,dimension)
...
%         end
    ...
    % Define array of point sources
        ...
        for l = 1:length(x)
            for k = 1:length(y)
                x_r = round(x(l)+(2*rand - 1)*w/4);
                y_r = round(y(k)+(2*rand - 1)*w/4);
                R_p( x_r , y_r ) = 2*pi*(rand-0.5);   % phase of point sources
                R_a( x_r , y_r ) = 1;                 % amplitude of point sources
            end
        end
...
%         [X,Y]=meshgrid(linspace(-dimension/2, dimension/2,dimension)); 
        x=linspace(-dimension(1)/2, dimension(1)/2,dimension(1));
        y=linspace(-dimension(2)/2, dimension(2)/2,dimension(2));
...
%         
    ...
    % is an intermediate vector
    if coherence_index >= dimension
        x=round(linspace(1,dimension(1),dimension(1))); % each pixel is a point source
        y=round(linspace(1,dimension(2),dimension(2)));
    else
...
%         K = X.^2 + Y.^2 < radius.^2;  %kernel
        if max(size(radius))==2
            K=(X./radius(1)).^2+(Y./radius(2)).^2<=1;
        else
            K=X.^2+Y.^2<=radius.^2; %creates Pupil 
        end
        % Create basis for extended source by defining multiple sources of the
        % required size, by convolving the kernel with the opint source array
        ES_a = ift0(ft0(K).*ft0(R_a)); %amplitude
        ...
        ES=ES_a.*exp(1i.*ES_p);
        range=1e-3./dimension;
        P=Makepupil(range,dimension,5e-3);
        P.field = ES;
